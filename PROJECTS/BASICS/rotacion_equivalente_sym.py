...
class RotacionEquivalente:
    ...
    def create_rotation_matrix(self):
        ...
        Ky = self.vector_K[1]
        Kz = self.vector_K[2]
        V0 = 1 - sympy.cos(self.angle)
        C = sympy.cos(self.angle)
        S = sympy.sin(self.angle)
...
class VectorEquivalente:
    def __init__(self, matriz_rotacion):
        ...
        try:
            self.angle = sympy.acos((Rk[0,0] + Rk[1,1] + Rk[2,2] - 1)/2)
        except:
            print("\nDOMINIO INVALIDO FUNCION ACOS")
            self.angle = 0

        cte = 1/(2*sympy.sin(self.angle))
